...
```{r}
calculate_conflict_metric <- function(storbmers_data, header_data)
{
  k = header_data$k
  all = 0
  for(i in 1:ncol(strobmers_data))
  {
    end_check = min(i + k, ncol(strobmers_data))
    kmer1_end = strobmers_data$kmer_1[i] + k - 1
    kmer2_end = strobmers_data$kmer_2[i] + k - 1
    max_overlap = 0
    for(j in i+1:end_check)
    {
      overlap = 0
      overlap = kmer1_end  - strobmers_data$kmer_1[j]
      if(overlap < 0)
          overlap = 0
      overlap = overlap + max(kmer2_end - storbmers_data$kmer_2[j], 0)
      if(header_data$n == 3)
      {
        overlap = overlap + max(0, strobmers_data$kmer_3[i] + k - 1 - strobmers_data$kmer_3[j])
      }
      max_overlap = max(overlap, max_overlap)
    }
    all = all + max_overlap
  }
  return (all / ncol(strobmers_data))
}
```
...
evaluate_sample <- function(strobmers_data, header_data, output_evaluation, type_kmer)
{
   ...
   if(type_kmer == 2)
   {
     ...
     output_evaluation[nrow(output_evaluation) + 1,] <- list(time_execution, ehits, diff_ehits, unique_k2, unique_hashes,
                                                          ...
                                                          hash_method, randstrobe_method, conflict, header_data$Sample)
   }else 
   {
     time_execution <- header_data$TimeExecution
...
if(kmer_type == 2)
{
  columns = c("TimeofExecution","E-hits_metric","E-hits-diff", "UniqueKmers_2", "UniqueHashes", "KS-test-diff",                                   "KS-test-kmer2","HashMethod", 
                "RandStrobeMethod", "Max-Conflict-Average", "Sample") 
}else 
{
