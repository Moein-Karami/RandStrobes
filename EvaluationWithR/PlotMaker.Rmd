...
draw_normalized_plot <- function(data_all_methods, new_data, name_now, i, font_y)
{
  plot_now <- ggplot(data_all_methods,
         ...
         ) + geom_bar(position="dodge", stat="identity") +  theme(title = element_text(size = 320), 
                                                                  
                                                                  plot.title = element_text( margin = margin(t = 120, b = 120)),
                                                                  axis.title=element_text(size = 200),
                                                                  axis.title.x = element_text(size = font_y, margin = margin(300, 300, 300, 300)),
                                                                                                                                                                                                           axis.title.y = element_text(size = font_y, margin = margin(360, 360, 360, 360)),
                                                                  axis.text.x = element_text(margin = margin(100, 100, 100, 100)),

                                                                  legend.text = element_text(size = 340),
                                                                  legend.title = element_text(size = 340),
                                                                  axis.text = element_text(size = 320, margin = margin(t = 60, b = 60)),
                                                                 legend.margin = margin(60, 60, 60, 60)) + 
    guides(fill=guide_legend(title="Hashes Function", override.aes = list(size = 100))) + scale_fill_discrete(labels = to_latex) + scale_x_discrete(labels = to_latex) +
theme(legend.text.align = 0) +   facet_wrap(~Comperator, nrow=1) + theme(plot.margin = unit(c(10,10,10,10), "lines"), strip.text = element_text(size = 480),    panel.spacing.x = unit(6, "cm")) + 
  ...
  png(filename=paste("../EvaluationResults/Figures/", paste("Normalized", name_now), ".png",sep = ""), width=30000, height= 12000)
  print(plot_now)
  dev.off()
}
```
...
data_all_methods$RandStrobeMethod[data_all_methods$RandStrobeMethod == "Traditional"] <- "Traditional"
...
for(i in 1:ncol(new_data))
{
  ...
  {
    data_all_methods <- data_all_methods[!grepl('Traditional', data_all_methods$RandStrobeMethod),]
    new_data <- data_all_methods
    new_data$Sample <- NULL
    new_data$RandStrobeMethod <- NULL
    new_data$HashMethod <- NULL
  ...
  plot_now <- ggplot(data_all_methods,
         aes(fill =  HashMethod, y = new_data[, i], x = RandStrobeMethod)
         ) + geom_bar(stat="identity", position = "dodge") + theme(title = element_text(size = 320), 
...
theme(legend.text.align = 0) +   facet_wrap(~Comperator, nrow=1) + theme(plot.margin = unit(c(10,10,10,10), "lines"), strip.text = element_text(size = 480),    panel.spacing.x = unit(6, "cm"))
  ...
  png(filename=paste("../EvaluationResults/Figures/", name_now, ".png",sep = ""), width=30000, height= 12000)
  print(plot_now)
  dev.off()
