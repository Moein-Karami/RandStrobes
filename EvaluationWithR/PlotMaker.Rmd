...
theme(legend.text.align = 0) +   facet_wrap(~Comperator, nrow=1) + theme(plot.margin = unit(c(10,10,10,10), "lines"), strip.text = element_text(size = 140),    panel.spacing.x = unit(2, "cm")) + 
  theme(legend.text.align = 0) + coord_cartesian(ylim = c(min(new_data[, i]), NA))
   plot_now <- plot_now + labs(title = "",
              x = "", y =  name_now)
  png(filename=paste("../EvaluationResults/Figures/", paste("Normalized", name_now), ".png",sep = ""), width=6000, height= 3000)
  print(plot_now)
  dev.off()
}
```
...
for(file_now in all_files)
{
  ...
  new_csv[1, 4] = new_csv[1, 4] / 1000000
  names(new_csv)[names(new_csv) == "E.hits.of.distance.between.strobe2.and.strobe1"] <- "E_d"
  names(new_csv)[names(new_csv) == "Conflicts"] <- "C"
  names(new_csv)[names(new_csv) == "E.hits.of.strobe2.positions"] <- "E_p"
  #print(ncol(new_csv))
  if(cnt == 1)
  {
...
theme(legend.text.align = 0) +   facet_wrap(~Comperator, nrow=1) + theme(plot.margin = unit(c(10,10,10,10), "lines"), strip.text = element_text(size = 140),    panel.spacing.x = unit(2, "cm"))
  ...
  png(filename=paste("../EvaluationResults/Figures/", name_now, ".png",sep = ""), width=6000, height= 3000)
  print(plot_now)
  dev.off()
