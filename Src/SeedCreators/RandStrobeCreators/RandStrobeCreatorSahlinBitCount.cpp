...
inline std::vector<Seed*> RandStrobeCreatorSahlinBitCount::create_seeds()
{
	...
	for (size_t i = 0; i < seq.size() - kmer_len - w_min - (n - 2) * w_max; i++)
	{
		...
		// {
			...
			best_value = __builtin_popcountll(curr_hash ^ hashes[best_choose]);

			for (size_t q = i + w_min + 1; q < std::min(i + w_max + 1, hashes.size()); q++)
			{
				// new_score = get_score(curr_hash, q);
				new_score = __builtin_popcountll(curr_hash ^ hashes[q]);
