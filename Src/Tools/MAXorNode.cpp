...
void MAXorNode::add(size_t ind, uint64_t val)
{
	...
	childs[(val & (1 << bit_ind)) != 0]->add(ind, val);
}
...
void MAXorNode::remove(size_t ind, uint64_t val)
{
	...
	childs[(val & (1 << bit_ind)) != 0]->remove(ind, val);
}
...
size_t MAXorNode::get_best_ind(uint64_t val, bool cmp)
{
	...
	if (childs[cmp ^ ((val & (1 << bit_ind)) != 0)]->is_empty() == false)
		return childs[cmp ^ ((val & (1 << bit_ind)) != 0)]->get_best_ind(val, cmp);
	else
		return childs[(cmp ^ ((val & (1 << bit_ind)) != 0)) ^ 1]->get_best_ind(val, cmp);
